...
var logFolding = testLogSteps
var logBranches = false
let logXMLParsing = false
...
func titleCorrection(var oldTitle:String) -> String{
    ...
    if ((characters[3] == "-" || characters[3] == "—" || characters[3] == "_") && characterIsNumber(characters[0])&&characterIsNumber(characters[1])&&characterIsNumber(characters[2])){
        if (characters[5] == " " && characters[3] != "_"){
            oldTitle.removeRange(  Range<String.Index>(start: oldTitle.startIndex, end: oldTitle.startIndex.advancedBy(6)))
        }
        else {
            oldTitle.removeRange(  Range<String.Index>(start: oldTitle.startIndex, end: oldTitle.startIndex.advancedBy(4)))
        }
        //oldTitle=oldTitle.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceCharacterSet())
    }
    return oldTitle
}
